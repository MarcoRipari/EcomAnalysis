...
```
app.py                      # Home: scelta del periodo (dal DB), esecuzione pipeline
core/
  config.py                 # CONFIG (mappe collezioni/nazioni, tassi cambio, IVA, layout colonne)
  engine.py                 # Lettura CSV/TXT + processDataset (vettorizzato pandas)
  db.py                     # DB SQLite incrementale: upsert DATASET/RESI, query per range date
  metrics.py                # calculateGlobalMetricsDetailed, computeChannelKPI, periodo
  reconciler.py             # Motore riconciliazione RESI a file (modalità legacy)
  ...
  pipeline.py               # Orchestrazione: run_pipeline (a file) e build_pipeline_from_db
  ui_helpers.py             # Helper Streamlit (column_config, guard pagina, period_labels)
pages/
  00_⬆️_Carica_Dati.py       # Upload incrementale DATASET/RESI nel DB
  01_📊_Dashboard.py
  02_📈_Y2Y_Generale.py       # Unico report con confronto opzionale a 3 vie
  03_🌳_Y2Y_Collezioni.py
  04_🔢_Y2Y_Codici.py
  05_♻️_Carryover.py
  06_👟_Taglie.py
  07_🔄_Analisi_Resi.py
  08_🔍_Log_Riconciliazione.py
  09_📦_Sell_Through.py
  10_🌍_Nazioni.py           # Comparativa per nazione selezionabile
requirements.txt
```
...
## ⚠️ Importante se hai già dati caricati con una versione precedente

Un bug nella v1 del layer DB (`_key_composito_series`/`_content_hash_series` in `core/db.py`)
usava per errore `lordoEUR` invece di `lordoSpedito` per calcolare la chiave di match e
l'hash di deduplicazione. Per una riga di un file RESI, `lordoEUR` è **sempre 0** (perché
`isReso=True` azzera il netto), quindi il match tramite chiave composita (quello pensato per
i siti proprietari, dove l'ordine RESI ha un numero diverso da quello DATASET) **non
funzionava mai**: righe che avrebbero dovuto riconciliarsi finivano erroneamente marcate come
"rimborso extra" (standalone) invece che aggiornare la riga DATASET esistente.

Corretto in questa versione. Se hai già caricato file RESI con la versione precedente, alcune
righe potrebbero essere finite come standalone per errore invece di essere state riconciliate
correttamente — **svuota il DB** (pulsante nella pagina "⬆️ Carica Dati" → Zona pericolosa) e
ricarica tutto da capo per essere sicuro che lo stato rifletta la logica corretta. Ricaricare
senza svuotare non basta: la firma dell'id è cambiata insieme al fix, quindi rischi righe
duplicate invece che sovrascritte.

## Etichette dei periodi nei report

Le colonne/etichette dei report non usano più l'anno di sistema (`datetime.date.today()`),
che era sganciato dal range di date effettivamente selezionato in home. Ora usano etichette
di posizione (`core/ui_helpers.period_labels()`): **"Range selezionato"**, **"Anno
precedente"**, **"Due anni precedenti"** — coerenti qualunque sia il range scelto (anche non a
cavallo dell'anno solare).

## Confronto a 3 vie — solo su Y2Y Generale

Come richiesto, l'unico report con il confronto opzionale a 3 vie è **Y2Y Generale**: una
checkbox aggiunge "Due anni precedenti", calcolato automaticamente spostando di un altro anno
il range scelto in home (stesso giorno/mese, gestendo il 29 febbraio) — non serve una terza
selezione di date. Gli altri report restano a 2 vie (Range selezionato vs Anno precedente).

## Pagina "🌍 Nazioni"

Nuova pagina: seleziona le nazioni presenti nel periodo (multiselect), e per ciascuna vedi
Fatturato Netto Totale (reale, comprensivo dei resi extra), Totale Ordini, Totale Paia Nette,
Paia Rese di ordini spediti nel range, Paia Rese di ordini spediti fuori dal range, % Reso, e
lo share del fatturato per brand (tab per nazione). Se hai selezionato un periodo di
confronto in home, vedi anche la tabella del periodo precedente e la variazione %.

Nota sulle pagine: rinominate con prefisso a due cifre (`00_`…`10_`) invece di una sola,
altrimenti l'ordinamento alfabetico di Streamlit avrebbe messo "10_" prima di "1_".

## Prestazioni upload (file da centinaia di migliaia di righe)

La v1 di `core/db.py` costruiva le righe da inserire con un ciclo `.iloc[i]` riga-per-riga:
accettabile su poche migliaia di righe, ma su 450.000 diventa lentissimo e pesante in RAM (fino
a far scadere/crashare la richiesta prima ancora di loggare l'esito). Riscritto in modo
vettoriale (costruzione via array numpy + `zip`, non `.iloc[i]`), con insert/update a blocchi
da 20.000 righe. Test di carico: ~358k righe DATASET in ~6.5s, upload idempotente dello stesso
file in ~4s, riconciliazione di ~143k RESI in ~6s (era nell'ordine dei minuti prima).

Anche la lettura del file è più rapida: individua separatore/encoding leggendo solo l'header
invece di fare fino a 8 parse completi del file per tentativi.

La pagina "⬆️ Carica Dati" ora ha un `try/except` attorno all'intero flusso per ogni file: un
errore su un file (malformato, memoria esaurita) non fa più sparire tutto senza traccia — mostra
l'errore specifico e passa al file successivo nello stesso batch.
